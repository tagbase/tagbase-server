...
INSERT INTO data_position (date_time, lat, lon, submission_id, tag_id) 
  VALUES(
    (SELECT date_time FROM moved_rows LIMIT 1),
    0.0,
    (SELECT variable_value FROM moved_rows LIMIT 1),
    (SELECT submission_id FROM moved_rows LIMIT 1),
    (SELECT tag_id FROM moved_rows LIMIT 1));
...
--      RETURNING a.*, cast(substring(variable_name, '(\d+)') AS int) AS bin_class)
-- UPDATE data_profile
