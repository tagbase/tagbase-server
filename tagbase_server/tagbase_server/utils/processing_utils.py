...
from tagbase_server.utils.db_utils import connect, detect_duplicate
...
def process_etuff_file(file, version=None, notes=None):
    ...
    duplicate = detect_duplicate(hash_sha256)
    logger.info(
        "Duplicate: %s",
        duplicate,
    )
    ...
    if duplicate:
        ...
        logger.info(
            "Data file '%s' successfully ingested into Tagbase DB.",
            submission_filename,
        )
    else:
        logger.info(
