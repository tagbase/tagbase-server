...
class TestTagController(BaseTestCase):
    def test_delete_sub(self):
        ...
        response = self.client.open(
            "/tagbase/api/v0.13.0/tags/{tag_id}/subs/{sub_id}".format(
                tag_id=3, sub_id=3
            ),
            method="DELETE",
            headers=headers,
    ...
    def test_delete_tag(self):
        ...
        response = self.client.open(
            "/tagbase/api/v0.13.0/tags/{tag_id}".format(tag_id=3),
            method="DELETE",
            headers=headers,
        )
        self.assert500(response, "Response body is : " + response.data.decode("utf-8"))
    ...
    def test_delete_tags(self):
        ...
        response = self.client.open(
            "/tagbase/api/v0.13.0/tags", method="DELETE", headers=headers
        )
        self.assert500(response, "Response body is : " + response.data.decode("utf-8"))
    ...
    def test_list_tags(self):
        ...
        response = self.client.open(
            "/tagbase/api/v0.13.0/tags", method="GET", headers=headers
        )
        self.assert500(response, "Response body is : " + response.data.decode("utf-8"))
