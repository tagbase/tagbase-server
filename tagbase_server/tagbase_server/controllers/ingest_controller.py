...
def ingest_get(file, notes=None, type=None, version=None):  # noqa: E501
    ...
    counter = 1
    for etuff in etuff_files:
        submission_filename = etuff[etuff.rindex("/") + 1 :]
        logger.info(
            "Processing etuff file %s of %s - %s",
            counter,
            len(etuff_files),
            submission_filename,
        )
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                # THIS IS WHERE TAR.GZ UNPACKING HAPPENED
    ...
    return Ingest200.from_dict(
        {
            "code": "200",
            "elapsed": elapsed,
            "message": "Ingested %s file(s) into Tagbase DB." % len(etuff_files),
        }
    )
